...
test('user creation requires unique email', function () {
    $this->authenticateAdmin();
    User::factory()->create(['email' => 'existing@saus.com']);

    $response = $this->postJson('/api/v1/users', [
        'first_name' => 'John',
        'last_name' => 'Doe',
        'email' => 'existing@saus.com',
        'username' => 'johndoe',
        'password' => 'password123',
        'password_confirmation' => 'password123',
    ]);
...
test('user update validates unique email', function () {
    $this->authenticateAdmin();
    $user1 = User::factory()->create(['email' => 'user1@saus.com']);
    $user2 = User::factory()->create(['email' => 'user2@saus.com']);

    $response = $this->putJson("/api/v1/users/{$user1->id}", [
        'first_name' => $user1->first_name,
        'last_name' => $user1->last_name,
        'email' => 'user2@saus.com',
...
test('user management handles edge case inputs', function () {
    ...
    foreach ($edgeCases as $case => $value) {
        $response = $this->postJson('/api/v1/users', [
            ...
            'email' => is_string($value) ? $value . '@saus.com' : 'test@saus.com',
...
test('user creation encrypts password', function () {
    ...
    $user = User::where('email', 'john@saus.com')->first();
